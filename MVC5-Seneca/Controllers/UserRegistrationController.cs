namespace MVC5_Seneca.Controllers
{
    ...
    //            {                                     
    //                var userDetails = new EntityModels.UserDetail();
    //                userDetails = db.OldUsers.Where(x => x.Name == CreateUserRegistrationViewModel.Name).FirstOrDefault();
    //                if (userDetails == null)
    //                {
    //                    var user = db.OldUsers.Create();
    ...
    //    private async Task EmailAdministrators(UserDetail user)
    //    {
    ...
    //        {
    //            var _user = (from u in db.OldUsers where u.Id == _role.User.Id select u).Single();
